...
### Added
- Added `scripts/dlp_build_runtime.py` and `scripts/build_daily_pack.py` as the supported fail-closed candidate/release boundary for Daily Lesson Pack artefacts.
- Added end-to-end CLI tests covering candidate staging, unresolved-context rejection and incomplete-evidence release blocking.
- Added explicit semantic relationship checks for Mathematics `Why` reasoning and Shared Reading paragraph/question/answer sets.
- Added deterministic explicit panel ownership so text crossing its intended panel is a hard release failure.
- Added the T3W8 Tuesday bypass known-failure regression and packaged it with the complete skill and independent QA component.

### Changed
- A request for a PowerPoint generated by a Python script now changes only the requested interface; it does not authorise an ad-hoc `python-pptx`, PptxGenJS or LibreOffice generation path.
- The root orchestrator and ChatGPT agent metadata now require the repository-owned build and release pipeline.
- Complete packages now contain the supported build runtime, and package validation checks that the runtime is present and importable.
- Modular routing regressions now explicitly verify that Python-script requests preserve repository runtime, current context/timetable authority and candidate status until release evidence passes.

### Reliability
- Candidate artefacts cannot become released output without the repository release authority returning PASS for the same deck and manifest hashes.
- Hand-written PASS files, stale review evidence and deck mutation after audit cannot promote a candidate.
- The T3W8 regression protects Morning Work retrieval scope, five-pair Numeracy architecture, 36 pt task/answer floors, meaningful `Why` reasoning, plausible Literacy distractors, accurate pronoun/grammar terminology, Shared Reading integrity, runtime/timetable fidelity and same-artefact release evidence.

## 3.8.0 — 6 September 2026

### Added
- Added the v3 pack-evidence workflow with canonical task text, requirement coverage and final-file bindings.
- Added reviewer receipts and separate semantic/rendered-review traces tied to generation run IDs.
- Added release evidence checks that distinguish scheduled applicability from broad component PASS assertions.

### Reliability
- Final release now depends on evidence bound to the complete pack rather than run-ID-only independence or report-only assertions.
- Required checks cannot be replaced by a generic component PASS, and the complete-pack release command remains the only classroom-ready authority.
