...
Read `references/shared-class-context-contract.md` before resolving a daily
pack. It defines the stable classroom context, the inputs that must be supplied
for the requested day and the memory-independence boundary.

Use sources in this order:
1. the user's current instruction;
2. the supplied shared-class/day context;
3. explicit maintained lesson-status exceptions;
4. connected calendar and authoritative timetable;
5. active unit/assessment plans;
6. current term/year overviews and curriculum maps;
7. bundled component-skill defaults.

Do not use chat memory, saved personal context, another account's Project
context or an unstated standing preference as evidence for timetable facts,
lesson focus, progression, copy quantity or classroom routines. A remembered
detail may identify a question to ask, but it must not silently supply the
answer. The repository package and the sources supplied for the current run
must be sufficient to reproduce the pack in a clean account.
...
pedagogy. The current orchestrator, component contracts, Mathematics canon and
regression records remain authoritative for content.
...
1. Resolve exact date, term, week, day, timetable and interruptions from the current run's supplied sources.
2. Resolve the shared-class contract, active sequence and explicit status exceptions.
3. For an overview-controlled Term 3 day, resolve the current overview fields before component generation. For Wednesday, require the supplied Wednesday day-level focus when the overview does not define one.
4. Verify and load the approved visual-only exemplar, then build one shared context object containing only information the components need: source provenance, lesson focus, curriculum boundary, authorised group/day allocation, current writing-toolkit stage, approved country contexts, time available and output constraints.
...
13. Send the complete assembled pack, context-source record, presentation-audit evidence and component-acceptance record to `dlp-pack-qa`.
...
If any required day input in `references/shared-class-context-contract.md` is
unresolved, ask one concise question and block release. Do not use remembered
context to fill the gap and do not label an improvised pack classroom-ready.
...
Apply the Tuesday and Thursday classroom-feedback regression records whenever the affected components change. The Tuesday regression record remains a component-standard benchmark with explicit content exclusions; the separate checksum-verified edited Tuesday PPTX is the visual-only exemplar.

Apply `examples/benchmarks/memory-independent-wednesday-regression.md` whenever
context routing, source precedence, overview handling, timetable resolution or
packaging changes.
