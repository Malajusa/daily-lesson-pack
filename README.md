...
Daily Lesson Pack 3.9 uses a small orchestrator plus independently owned teaching components. The architecture is designed to prevent a change to one component from silently changing another, to produce the same-quality pack in a clean account without relying on chat memory, and to prevent ad-hoc presentation generation from bypassing repository-owned QA and release controls.
...
The supported Daily Lesson Pack finalisation entry point is:

```bash
python scripts/build_daily_pack.py --request request.json --context context.json --content content.json --component-record component-record.json --deck pack.pptx --manifest manifest.json --out build/run
```

That command validates the resolved runtime inputs and component evidence, stages a **candidate**, and only promotes the pack through the repository-owned final audit when complete independent release evidence is supplied. A request for a PowerPoint generated by a Python script does not authorise a separate `python-pptx`, PptxGenJS or LibreOffice workflow that recreates lesson generation or self-certifies release.

The skill does not read lesson facts from chat memory. Stable shared-class
routines live in `references/shared-class-context-contract.md`. When a maintained
plan is explicitly selected, `references/term-week-day-planning-contract.md`
resolves Level A term scope and the Level B weekly sequence before the actual
timetable places the learning into a Level C day plan. Otherwise each run supplies
the requested day's current focus directly. Lesson-status records are
exception-only: missing status advances normally and never asserts mastery.
...
├── references/                           # Shared visual, QA, planning and Mathematics standards
├── planning/                              # Explicitly selected maintained Level A/B plans
│   └── 2026/term-4/                      # Year 4/5 Term 4 plan + exception-only status
├── assets/visual-exemplars/              # Checksum-verified visual-only benchmark
├── scripts/                              # Build runtime, packaging and QA
├── docs/
│   ├── DEVELOPMENT.md
...
The bundled 2026 Term 4 Year 4/5 plan is a maintained creator/class planning
source, not a universal Year 4/5 default. It is consumed only when the current
run selects that plan or an authoritative maintained class context binds it.
...
`examples/benchmarks/t3w8-tuesday-bypass-known-failure.md` records the failure class where a request for a Python-generated PowerPoint could otherwise bypass the repository runtime, timetable/context provenance, component evidence and final release gates. Apply it whenever generation routing, packaging or release certification changes.
...
- Treat generated artefacts as candidates until the repository-owned release bundle passes for the exact deck and manifest hashes being delivered.
...
6. Stage it through `scripts/build_daily_pack.py`; do not use an independent presentation generator as a replacement workflow.
7. Run the generic repository audits, resolve every warning individually and complete independent semantic and rendered visual reviews.
8. Aggregate all same-hash evidence with `scripts/audit_release_bundle.py` and promote only that exact candidate.
9. Compare against the relevant classroom-feedback benchmarks, including the T3W8 bypass record when routing/release behaviour changes.
10. Rebuild and validate the complete package and affected component package.
11. Review the diff for unintended rule loss or contradiction.
12. Merge only after representative classroom regression and package verification pass.
13. Synchronise/register the accepted component skill in the production host where supported.

## Current version

**3.9.0 — enforces the repository-owned candidate/release pipeline, prevents Python-script presentation requests from bypassing Daily Lesson Pack routing and QA, packages the supported runtime, and adds T3W8 bypass regression coverage plus end-to-end release-boundary tests.**
