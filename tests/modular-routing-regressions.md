...
## 15. Python-script request cannot bypass the repository runtime

Request a Daily Lesson Pack as a PowerPoint generated by a Python script.

PASS when:
- the request remains a Daily Lesson Pack request and still uses the repository runtime;
- any thin Python entry script delegates to `scripts/build_daily_pack.py` rather than rebuilding pedagogy, layout or QA independently;
- runtime timetable, overview and explicit overrides remain authoritative inputs;
- the produced artefact is treated as a candidate until independent repository release evidence passes;
- an ad-hoc presentation generator cannot self-certify the deck as classroom-ready.